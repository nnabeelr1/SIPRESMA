...
<head>
    ...
    <style>
        ...
        /* Logic Warna IPK */
        .badge-ipk-high { background: #dcfce7; color: #15803d; border: 1px solid #bbf7d0; } /* > 3.00 */
        .badge-ipk-mid  { background: #fef9c3; color: #854d0e; border: 1px solid #fef08a; } /* 2.50 - 3.00 */
        .badge-ipk-low  { background: #fee2e2; color: #b91c1c; border: 1px solid #fecaca; } /* < 2.50 */
    ...
    <div class="container mb-5 pb-5">
        ...
        <div class="page-header animate-entry" style="animation-delay: 0.1s;">
            <div>
                ...
                <p class="page-subtitle">Monitoring data akademik mahasiswa aktif.</p>
            </div>
            
            <div>
                <a href="create.php" class="btn-gradient-primary">
                    <iconify-icon icon="solar:user-plus-bold" class="fs-5"></iconify-icon> Tambah Mahasiswa
                </a>
            </div>
        </div>
                ...
                <table class="table table-custom mb-0">
                    <thead>
                        <tr>
                            ...
                            <th width="8%" class="text-center">IPK</th>
                    ...
                    <tbody>
                        ...
                        $query = mysqli_query($koneksi, "
                            SELECT m.*, p.nama_prodi, p.jenjang, d.nama_lengkap as nama_dosen 
                            FROM mahasiswa m 
                        ...
                        if(mysqli_num_rows($query) == 0) {
                            echo "<tr><td colspan='7'>
                                <div class='empty-state'>
                                    ...
                                    <h6 class='fw-bold text-dark'>Belum Ada Data Mahasiswa</h6>
                                    <p class='text-muted small'>Silakan tambah data mahasiswa baru.</p>
                                </div>
                            </td></tr>";
                        }
                        ...
                        while ($row = mysqli_fetch_array($query)) {
                            ...
                            if($ipk < 2.50) { $badge_ipk = 'badge-ipk-low'; }
                            elseif($ipk < 3.00) { $badge_ipk = 'badge-ipk-mid'; }
                            else { $badge_ipk = 'badge-ipk-high'; }
                        ?>
                            <tr>
                                <td class="text-center fw-bold text-muted"><?php echo $no++; ?></td>
                                
                                <td>
                                    <span class="text-mono fw-bold text-dark"><?php echo $row['nim']; ?></span>
                                    <div class="small text-muted mt-1">Angkatan <?php echo $row['angkatan']; ?></div>
                                </td>
                                ...
                                <td>
                                    <span class="badge-pill badge-prodi">
                                        <?php echo $row['jenjang']; ?> - <?php echo $row['nama_prodi']; ?>
                                    </span>
                                </td>
                                
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <iconify-icon icon="solar:user-id-bold-duotone" class="text-primary"></iconify-icon>
                                        <span class="small text-muted"><?php echo $row['nama_dosen']; ?></span>
                                    </div>
                                </td>

                                <td class="text-center">
                                    <span class="badge-pill <?php echo $badge_ipk; ?>">
                                        <?php echo $ipk; ?>
                                    </span>
                                </td>
                                ...
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-2">
                                        <a href="update.php?nim=<?php echo $row['nim']; ?>" class="btn-action-soft btn-soft-warning" title="Edit">
                                            <iconify-icon icon="solar:pen-bold" class="fs-5"></iconify-icon>
                                        </a>
                                        <a href="delete.php?nim=<?php echo $row['nim']; ?>" class="btn-action-soft btn-soft-danger" onclick="return confirm('Hapus data mahasiswa <?php echo $row['nama_lengkap']; ?>?')" title="Hapus">

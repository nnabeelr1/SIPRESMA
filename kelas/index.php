...
<head>
    ...
    <title>Data Kelas - SIPRESMA</title>
    ...
    <style>
        ...
        .badge-kelas { background: #f0fdf4; color: #15803d; border: 1px solid #dcfce7; }
        .text-mono { font-family: monospace; color: var(--text-muted); }
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <div>
                <h2 class="page-title">Jadwal Kelas</h2>
                <p class="page-subtitle">Manajemen pembukaan kelas dan jadwal perkuliahan.</p>
            </div>
            <div class="d-flex gap-2">
                <a href="../dashboard/index.php" class="btn-clean"><iconify-icon icon="solar:arrow-left-linear"></iconify-icon> Dashboard</a>
                <a href="create.php" class="btn-primary-soft"><iconify-icon icon="solar:add-circle-bold"></iconify-icon> Buka Kelas</a>
            </div>
        </div>
                ...
                <table class="table table-custom mb-0">
                    ...
                    <tbody>
                        ...
                        $query = mysqli_query($koneksi, "
                            SELECT k.*, m.nama_mk, m.kode_mk, m.sks, d.nama_lengkap 
                            FROM kelas k
                        ...
                        if(mysqli_num_rows($query) == 0) {
                            echo "<tr><td colspan='6'><div class='empty-state'><iconify-icon icon='solar:black-hole-linear' style='font-size: 3rem; opacity: 0.5; margin-bottom: 1rem;'></iconify-icon><p>Belum ada kelas dibuka.</p></div></td></tr>";
                        }
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <span class="fw-bold text-dark d-block"><?php echo $data['nama_mk']; ?></span>
                                    <small class="text-mono"><?php echo $data['kode_mk']; ?> • <?php echo $data['sks']; ?> SKS</small>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <iconify-icon icon="solar:user-id-bold" class="text-muted"></iconify-icon>
                                        <span class="text-dark small"><?php echo $data['nama_lengkap']; ?></span>
                                    </div>
                                </td>
                                <td class="text-center"><span class="badge-pill badge-kelas"><?php echo $data['nama_kelas']; ?></span></td>
                                <td>
                                    <span class="fw-bold text-dark small d-block"><?php echo $data['hari']; ?></span>
                                    <span class="text-muted small"><?php echo date('H:i', strtotime($data['jam_mulai'])) . ' - ' . date('H:i', strtotime($data['jam_selesai'])); ?></span>
                                </td>
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-2">
                                        <a href="update.php?id=<?php echo $data['id_kelas']; ?>" class="btn-icon-soft btn-edit"><iconify-icon icon="solar:pen-bold"></iconify-icon></a>
                                        <a href="delete.php?id=<?php echo $data['id_kelas']; ?>" class="btn-icon-soft btn-delete" onclick="return confirm('Hapus kelas ini?')"><iconify-icon icon="solar:trash-bin-trash-bold"></iconify-icon></a>
                                    </div>
                                </td>
                            </tr>
                        <?php } ?>

...
if (isset($_POST['simpan'])) {
    $user = $_POST['username'];
    $pass = $_POST['password']; // Sebaiknya di-hash (md5/password_hash) di real production
    $role = $_POST['role'];
    
    // Cek username duplikat
    $cek = mysqli_query($koneksi, "SELECT username FROM user WHERE username = '$user'");
    if(mysqli_num_rows($cek) > 0){
        echo "<script>alert('❌ Username sudah terdaftar!');</script>";
    } else {
        $simpan = mysqli_query($koneksi, "INSERT INTO user (username, password, role) VALUES ('$user', '$pass', '$role')");
        if ($simpan) { 
            echo "<script>alert('✅ Berhasil menambahkan user!'); window.location='index.php';</script>"; 
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .card-modern {
            background: rgba(255, 255, 255, 0.8); /* Agak lebih solid drpd tabel biar form jelas */
            ...
            max-width: 500px;
            margin: 2rem auto; /* Center alignment */
        ...
        /* --- BUTTONS --- */
        /* Tombol Simpan (Gradient Green - Sama kaya Index) */
    ...
    <div class="container mb-5">
        ...
        <div class="card-modern animate-entry">
            <div class="text-center mb-4">
                ...
                <h4 class="fw-bold mb-1">Tambah Admin Baru</h4>
                <p class="text-muted small">Buat akun login untuk pengelola sistem.</p>
            </div>

            <form method="POST">
                <div class="mb-3">
                    <label class="form-label">Username</label>
                    <input type="text" name="username" class="form-control" placeholder="Contoh: admin_prodi" required autofocus>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="form-text mt-2 small text-muted">
                        ...
                        Akun Mahasiswa & Dosen dibuat otomatis di Data Master.
                    </div>
                </div>

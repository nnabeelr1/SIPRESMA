...
$nama_user = "Administrator"; // Default name for admin
...
<head>
    ...
    <style>
        ...
        .alert-glass-info {
            ...
            color: #1e40af;
            border-radius: 16px; padding: 1rem 1.5rem; font-size: 0.9rem;
        ...
        .badge-dosen { background: #dcfce7; color: #15803d; border: 1px solid #bbf7d0; }
        .badge-mhs   { background: #dbeafe; color: #1e40af; border: 1px solid #bfdbfe; }
    </style>
</head>
<body>
        ...
        <div class="page-header animate-entry" style="animation-delay: 0.1s;">
            ...
            <div>
                <a href="create.php" class="btn-gradient-primary">
                    <iconify-icon icon="solar:user-plus-bold" class="fs-5"></iconify-icon> Tambah User
                </a>
            </div>
        </div>

        <div class="alert-glass-info animate-entry" style="animation-delay: 0.2s;">
            <iconify-icon icon="solar:info-circle-bold-duotone" class="fs-4 mt-1"></iconify-icon>
            <div>
                <strong class="d-block mb-1">Informasi Sistem</strong>
                Halaman ini khusus untuk mengelola akun login (Username & Password). 
                ...
                <table class="table table-custom mb-0">
                    ...
                    <tbody>
                        ...
                        while ($data = mysqli_fetch_array($query)) {
                            // Logic Badge Warna
                            if($data['role']=='admin') { 
                                $badge='badge-admin'; $icon='solar:shield-user-bold'; $role='Administrator'; 
                            } elseif($data['role']=='dosen') { 
                                $badge='badge-dosen'; $icon='solar:user-id-bold'; $role='Dosen Pengajar'; 
                            } else { 
                                $badge='badge-mhs'; $icon='solar:square-academic-cap-bold'; $role='Mahasiswa'; 
                            }
                        ?>
                            <tr>
                                ...
                                <td>
                                    <span class="fw-bold text-dark"><?php echo $data['username']; ?></span>
                                </td>
                                
                                <td>
                                    <span class="badge-pill <?php echo $badge; ?>">
                                        <iconify-icon icon="<?php echo $icon; ?>"></iconify-icon> <?php echo $role; ?>
                                    </span>
                                </td>
                                
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-2">
                                        <a href="update.php?id=<?php echo $data['id_user']; ?>" class="btn-action-soft btn-soft-warning" title="Edit">
                                            <iconify-icon icon="solar:pen-bold" class="fs-5"></iconify-icon>
                                        </a>
                                        
                                        <?php if($data['username'] != $_SESSION['username']) { ?>
                                            <a href="delete.php?id=<?php echo $data['id_user']; ?>" class="btn-action-soft btn-soft-danger" onclick="return confirm('Hapus user ini?')" title="Hapus">
                                                <iconify-icon icon="solar:trash-bin-trash-bold" class="fs-5"></iconify-icon>
                                            </a>
                                        <?php } else { ?>
                                             <button class="btn-action-soft" style="opacity: 0.3; cursor: not-allowed;" disabled>
                                                <iconify-icon icon="solar:lock-keyhole-bold" class="fs-5"></iconify-icon>
                                             </button>
                                        <?php } ?>
                                    </div>
                                </td>
